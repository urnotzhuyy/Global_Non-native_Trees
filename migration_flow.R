...
for (id in 1: length(nonnative_all)) {
  ...
  if(sp %in% nonnative_list){
    ...
    sp_file_des <- read.table(paste0(path_folder, "aggregate_data/tree_data_nonnative_final_update/", sp, "_nonnative.txt"), header = TRUE)
    # destination
    ...
    # non-native data (destination continent)
    sp_file_des <- read.table(paste0(path_folder, "aggregate_data/tree_data_nonnative_final_update/bgci_part/", sp, "_nonnative.txt"), header = TRUE)
    # destination
